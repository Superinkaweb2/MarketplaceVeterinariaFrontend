            ...
            <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 p-8 md:p-12 text-center space-y-8 animate-in zoom-in duration-500">
                ...
                <div className="space-y-4">
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">
                        {syncing ? 'Activando tu plan...' : '¡Pago Completado!'}
                    </h1>
                    <p className="text-slate-500 font-medium">
                        {syncing
                            ? 'Estamos sincronizando tu pago y actualizando tu suscripción.'
                            : syncError
                                ? 'El pago fue procesado pero hubo un problema al activar tu plan.'
                ...
                <div className="flex flex-col gap-3">
                    <Button onClick={() => navigate('/portal/cliente/mascotas')} className="w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-primary/20">
                        <LayoutDashboard size={18} />
                        Ir a Mis Mascotas
                    </Button>
                    <Button variant="outline" onClick={() => navigate('/portal/cliente/suscripcion')} className="w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 border-2">
                        Ver mi Suscripción
                        <ArrowRight size={18} />
                    </Button>
                </div>
